...
static _bool Vacuum_Object(CKirby* pKirby, _float fTimeDelta)
{
	...
	{
		if (nullptr != GAMEINSTANCE Get_List(*GAMEINSTANCE Get_CurrentLevelID(), TEXT("Layer_BattleDee")))
		{
			for (auto& pObject : *GAMEINSTANCE Get_List(*GAMEINSTANCE Get_CurrentLevelID(), TEXT("Layer_BattleDee")))
			{
				if (static_cast<CPhysXObject*>(pObject)->Get_PhyXState() != PO_NORMAL)
					continue;
...
static void Copy_Star(CTransform* pTransformCom)
{
	CMultiEffect::MULTI_FX_DESC FXDesc{};
...
static void Bbong_FX(_float fTimeDelta, CTransform* pTransformCom)
{
	...
	if (.2f < fBbongTime)
	{
		...
		//if (FAILED(CGameInstance::Get_Instance()->Add_Clone(*CGameInstance::Get_Instance()->Get_CurrentLevelID(), TEXT("Layer_Effect"), TEXT("Prototype_GameObject_client orbit test"), &SingleFXDesc)))
		//	return;
...
static void SwordSpinCharge(CTransform* pTransformCom, CKirby* pKirby)
{
	...
	//FXDesc.vInitScale = { 2.f, 2.f, 2.f };
	//if (FAILED(CGameInstance::Get_Instance()->Add_Clone(*CGameInstance::Get_Instance()->Get_CurrentLevelID(), TEXT("Layer_Effect"), TEXT("Prototype_GameObject_YW Sword Oras"), &FXDesc)))
	//	return;
	//pKirby->Add_Effect(static_cast<CEffect*>(CGameInstance::Get_Instance()->Get_List(*CGameInstance::Get_Instance()->Get_CurrentLevelID(), TEXT("Layer_Effect"))->back()));

...
HRESULT CLevel_PartTime::Initialize()
{
...
void CLevel_PartTime::Tick(_float fTimeDelta)
{
	__super::Tick(fTimeDelta);
	m_fAccDelta += fTimeDelta;
}

HRESULT CLevel_PartTime::Render()
{
	...
	wsprintf(szFPS, TEXT("Level PartTime, %d FPS"), m_iFPS);
...
HRESULT CLevel_PartTime::Ready_Lights()
{
...
HRESULT CLevel_PartTime::Ready_Layer_UI(const wstring& _wstrLayerTag)
{
	...
	CHECK_FAILED(hr);
	//hr = m_pGameInstance->Add_Clone(m_iLevel, _wstrLayerTag, TEXT("Prototype_GameObject_UI_PartTimeDee"));
	//CHECK_FAILED(hr);
	return S_OK;
}
...
HRESULT CLevel_PartTime::Ready_Map()
{
	...
	{
		MSG_BOX(TEXT("Failed to open : PartTime_Map.txt"));
		return E_FAIL;
	}
...
HRESULT CLevel_PartTime::Ready_Triggers()
{
	...
	if (fileInput.is_open() == false)
	{
		MSG_BOX(TEXT("Failed to open : PartTime_Triggers.txt"));
		return E_FAIL;
	}
...
HRESULT CLevel_PartTime::Ready_Dees()
{
	...
	if (fileInput.is_open() == false)
	{
		MSG_BOX(TEXT("Failed to open : PartTime_Monsters.txt"));
		return E_FAIL;
	}
	...
	for (_uint i = 0; i < iNumObjects; i++)
	{
		...
		if (L"KirbyPartTimer" == tempDesc.wstrModelName)
		{
...
HRESULT CLevel_PartTime::Ready_Items()
{
	...
	if (fileInput.is_open() == false)
	{
		MSG_BOX(TEXT("Failed to open : PartTime_Items.txt"));
		return E_FAIL;
	}
...
HRESULT CLevel_PartTime::Ready_Kickables()
{
	...
	if (fileInput.is_open() == false)
	{
		MSG_BOX(TEXT("Failed to open : PartTime_Kickables.txt"));
		return E_FAIL;
	}
...
HRESULT CLevel_PartTime::Add_EnvMap()
{
	HRESULT hr;
...
CLevel_PartTime* CLevel_PartTime::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{

	CLevel_PartTime* pInstance = new CLevel_PartTime(pDevice, pContext);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX(TEXT("Failed To Create : CLevel_PartTime"));

...
CBabyDuck::CBabyDuck(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CMonster{ pDevice, pContext }
{
}

CBabyDuck::CBabyDuck(const CBabyDuck& rhs)
	: CMonster(rhs)
{
}
...
HRESULT CBabyDuck::Initialize(void* pArg)
{
...
_int CBabyDuck::Tick(_float fTimeDelta)
{
...
void CBabyDuck::Late_Tick(_float fTimeDelta)
{
...
HRESULT CBabyDuck::Render_LightDepth()
{
...
void CBabyDuck::Collision(CCollisionCenter::CONTENT_TYPE eContent, CPhysXObject* pObject)
{
	
}

void CBabyDuck::Change_State(ANIM_STATE eState, _float _fAnimSpeed, _bool _bLoop, _bool _bInterpolation)
{
	m_pFSM->ChangeState((_uint)eState, _fAnimSpeed, _bLoop, _bInterpolation);
}

_bool CBabyDuck::IsAnimFinished()
{
	return m_pModelCom->IsFinished();
}

_bool CBabyDuck::IsAnimFinished(_uint iCurrentAnimIndex)
{
	return m_pModelCom->IsFinished(iCurrentAnimIndex);
}

HRESULT CBabyDuck::Add_Components()
{
	HRESULT hr(S_OK);
	...
	hr = __super::Add_Component(TEXT("Prototype_Component_Model_SurprisedBoardRed"),
		TEXT("Com_Model_RED"), (CComponent**)&m_pModelCom);
	CHECK_FAILED(hr);
...
HRESULT CBabyDuck::Add_HitBoxes(_float3 vPos)
{
	HRESULT hr(S_OK);
	...
	CHECK_FAILED(hr);
	Set_BodyCollider(COLLIDER_SPHERE, 0.f, 2.f, 2.f);

	return S_OK;
}

HRESULT CBabyDuck::Bind_ShaderResources()
{
...
void CBabyDuck::SetUp_FSM()
{
	...
	FSM_Desc.iState = IDLE;
	FSM_Desc.pModel = &m_pModelCom;
	m_pFSM->Initialize(&FSM_Desc);
}

CBabyDuck* CBabyDuck::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CBabyDuck* pInstance = new CBabyDuck(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Create : CBabyDuck"));
...
CGameObject* CBabyDuck::Clone(void* pArg)
{
	CBabyDuck* pInstance = new CBabyDuck(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Clone : CBabyDuck"));
		Safe_Release(pInstance);
	}

...
void CFinalBoss_Appear_State::OnStateUpdate(CGameObject* pGameObject, _float fTimeDelta)
{
	...
	if (0.7f < pFinalBoss->Get_AnimRatio())
	{
		if (false == m_bSound2)
		{
			...
			BossNameDesc.fScale = 1.f;
			BossNameDesc.vPosition = pTransformCom->Get_State_Vector(CTransform::STATE_POSITION) + pTransformCom->Get_State_Vector(CTransform::STATE_LOOK) * 4.f + pTransformCom->Get_State_Vector(CTransform::STATE_UP) * 0.3f;

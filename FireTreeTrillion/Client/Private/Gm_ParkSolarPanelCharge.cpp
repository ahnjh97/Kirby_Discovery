...
_int CGm_ParkSolarPanelCharge::Tick(_float fTimeDelta)
{
	...
	*/
	...
	if (m_pGameInstance->Get_DIKeyState(DIK_A, KEY_PRESS) && STATE_OFFWAIT == m_eAnimState)
	{
		m_eAnimState = STATE_CHARGE;
		m_pModelCom->Set_Animation(STATE_CHARGE, 60.f, FALSE);
	}
	
	//충전 중 상태에서 키꾹 해제 할 경우, 충전 해제 처리

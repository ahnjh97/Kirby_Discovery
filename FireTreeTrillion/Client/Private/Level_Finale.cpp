...
void CLevel_Finale::Tick(_float fTimeDelta)
{
	__super::Tick(fTimeDelta);
	m_fAccDelta += fTimeDelta;
}

HRESULT CLevel_Finale::Render()
{
	...
	wsprintf(szFPS, TEXT("Level Finale, %d FPS"), m_iFPS);
...
HRESULT CLevel_Finale::Ready_Lights()
{
	//// 예시코드 1 : 태양광
...
HRESULT CLevel_Finale::Ready_Layer_Camera(const wstring& strLayerTag)
{
...
#pragma region 처음 빌딩들
	...
	roadGrouperDesc = {};
	roadGrouperDesc.eRoadType = RTYPE_BUILDINGC;
	roadGrouperDesc.eMoveCommand = MOVECMD_STOP;

	InitMat = _float4x4::Identity;
	InitMat.Translation({ 268.1f, -15.5f, -22.9f });
	CUtils::Rotation(InitMat, CUtils::Make_Quat_FromDir({ 1.f, .08f, -.05f }));
	roadGrouperDesc.matWorld = InitMat;
	...
	InitMat = _float4x4::Identity;
	InitMat.Translation({389.f, -12.5f, -19.f});

	CUtils::Rotation(InitMat, CUtils::Make_Quat_FromDir({ .93f, -.25f, -.27f }));
	roadGrouperDesc.matWorld = InitMat;

	roadGrouperDesc.vDestPos = {389.2f, -12.5f, -19.1f };
	roadGrouperDesc.vDestDir = { 1.f, .06f, -.13f };
	...
	//도로 A
	roadGrouperDesc = {};
	roadGrouperDesc.eRoadType = RTYPE_ROADA;
	roadGrouperDesc.eMoveCommand = MOVECMD_COLLIDE;
...
#pragma region 회전 빌딩 길
	...
	//InitMat = _float4x4::Identity;
	//InitMat.Translation({ 1083.f, -190.f,-158.f });
	//CUtils::Rotation(InitMat, CUtils::Make_Quat_FromDir({ .98f, .22f, -.03f }));
	//roadGrouperDesc.matWorld = InitMat;

	//roadGrouperDesc.vDestPos = { 1100.311f, -196.760f, -164.158f };
	//roadGrouperDesc.vDestDir = { .9f, .22f, -.03f };
	...
	//InitMat = _float4x4::Identity;
	//InitMat.Translation({ 1234.6f,-170.001f,-141.846f });
	//CUtils::Rotation(InitMat, CUtils::Make_Quat_FromDir({ .88f, .41f, .25f }));
	////CUtils::Rotation(InitMat, Quaternion::CreateFromYawPitchRoll({ ToRadian(-59.f), ToRadian(59.f), ToRadian(-38.f) }));
	//roadGrouperDesc.matWorld = InitMat;
	...
	InitMat = _float4x4::Identity;
	InitMat.Translation({ 1362.651f, -123.036f,-111.231f });
	CUtils::Rotation(InitMat, CUtils::Make_Quat_FromDir({ .92f, .39f, .06f }));
	roadGrouperDesc.matWorld = InitMat;
	...
	InitMat.Translation({ 1488.866f, -98.833f,-140.250f });
	CUtils::Rotation(InitMat, CUtils::Make_Quat_FromDir({ .95f, .25f, -.18f }));
	roadGrouperDesc.matWorld = InitMat;
...
HRESULT CLevel_Finale::Ready_Map()
{
	string strFileName = "../../../objects_txt/Finale_Map.txt";
	ifstream fileInput(strFileName, ios::binary);
	if (fileInput.is_open() == false)
	{
		MSG_BOX(TEXT("Failed to open : Finale_Map.txt"));
		return E_FAIL;
	}
...
HRESULT CLevel_Finale::Ready_Triggers()
{

	string strFileName = "../../../objects_txt/Finale_Triggers.txt";
	ifstream fileInput(strFileName, ios::binary);
	if (fileInput.is_open() == false)
	{
		MSG_BOX(TEXT("Failed to open : Finale_Triggers.txt"));
		return E_FAIL;
	}
...
HRESULT CLevel_Finale::Ready_Monsters()
{

	string strFileName = "../../../objects_txt/Finale_Monsters.txt";

	ifstream fileInput(strFileName, ios::binary);
	if (fileInput.is_open() == false)
	{
		MSG_BOX(TEXT("Failed to open : Finale_Monsters.txt"));
		return E_FAIL;
	}
...
HRESULT CLevel_Finale::Ready_Items()
{

	string strFileName = "../../../objects_txt/Finale_Items.txt";

	ifstream fileInput(strFileName, ios::binary);
	if (fileInput.is_open() == false)
	{
		MSG_BOX(TEXT("Failed to open : Finale_Items.txt"));
		return E_FAIL;
	}
...
HRESULT CLevel_Finale::Ready_Kickables()
{
	string strFileName = "../../../objects_txt/Finale_Kickables.txt";

	ifstream fileInput(strFileName, ios::binary);
	if (fileInput.is_open() == false)
	{
		MSG_BOX(TEXT("Failed to open : Finale_Kickables.txt"));
		return E_FAIL;
	}
...
HRESULT CLevel_Finale::Ready_Objects()
{
	...
	if (fileInput.is_open() == false)
	{
		MSG_BOX(TEXT("Failed to open : Finale.txt"));
		return E_FAIL;
	}
...
CLevel_Finale* CLevel_Finale::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	...
	{
		MSG_BOX(TEXT("Failed To Create : CLevel_Finale"));

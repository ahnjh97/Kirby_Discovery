...
HRESULT CLevel_Racing::Initialize()
{
...
HRESULT CLevel_Racing::Ready_Lights()
{
	//// 예시코드 1 : 태양광
...
HRESULT CLevel_Racing::Ready_Layer_Camera(const wstring& strLayerTag)
{
...
HRESULT CLevel_Racing::Ready_Layer_UI(const wstring& _wstrLayerTag)
{
	//모든 HUD를 준비
...
HRESULT CLevel_Racing::Ready_Map(_float fXOffset, _float fZOffset)
{
	...
	{
		MSG_BOX(TEXT("Failed to open : Racing_Map.txt"));
		return E_FAIL;
	}
...
HRESULT CLevel_Racing::Ready_Triggers(_float fXOffset, _float fZOffset)
{
	...
	if (fileInput.is_open() == false)
	{
		MSG_BOX(TEXT("Failed to open : Racing_Triggers.txt"));
		return E_FAIL;
	}
...
HRESULT CLevel_Racing::Ready_Monsters(_float fXOffset, _float fZOffset)
{
	...
	if (fileInput.is_open() == false)
	{
		MSG_BOX(TEXT("Failed to open : Racing_Monsters.txt"));
		return E_FAIL;
	}
...
HRESULT CLevel_Racing::Ready_Items(_float fXOffset, _float fZOffset)
{
	...
	if (fileInput.is_open() == false)
	{
		MSG_BOX(TEXT("Failed to open : Racing_Items.txt"));
		return E_FAIL;
	}
...
HRESULT CLevel_Racing::Ready_Kickables(_float fXOffset, _float fZOffset)
{
	...
	if (fileInput.is_open() == false)
	{
		MSG_BOX(TEXT("Failed to open : Racing_Kickables.txt"));
		return E_FAIL;
	}
...
HRESULT CLevel_Racing::Ready_Objects(_float fXOffset, _float fZOffset)
{
	...
	if (fileInput.is_open() == false)
	{
		MSG_BOX(TEXT("Failed to open : Racing.txt"));
		return E_FAIL;
	}
	...
	while (!fileInput.eof())
	{
		...
		else if ("BoxWood" == strModelName || "BoxPlastic" == strModelName)
		{
			strModelName += "_Anim";
			tDesc.wstrModelName = CUtils::StrToWstr(strModelName);
			if (FAILED(m_pGameInstance->Add_Clone(eLevel, TEXT("Layer_Box"), TEXT("Prototype_GameObject_Box"), &tDesc)))
				continue;
		}
	}
	fileInput.close();
...
HRESULT CLevel_Racing::Add_EnvMap()
{
	HRESULT hr;
...
CLevel_Racing* CLevel_Racing::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CLevel_Racing* pInstance = new CLevel_Racing(pDevice, pContext);

	if (FAILED(pInstance->Initialize()))
	{
		MSG_BOX(TEXT("Failed To Create : CLevel_Racing"));
...
HRESULT CLevel_Racing::Load_FileData(const string& _strFilePath, FILE_TYPE _eFileType, const wstring& _wstrLayerTag)
{

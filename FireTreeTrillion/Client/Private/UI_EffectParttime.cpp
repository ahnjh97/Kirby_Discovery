#include "stdafx.h"
#include "UI_EffectParttime.h"


CUI_EffectParttime::CUI_EffectParttime(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
	: CUIObject{ _pDevice, _pContext }
{
}

CUI_EffectParttime::CUI_EffectParttime(const CUI_EffectParttime& rhs)
	: CUIObject{ rhs }
{
}

HRESULT CUI_EffectParttime::Initialize_Prototype()
{
	
	return S_OK;
}

HRESULT CUI_EffectParttime::Initialize(void* pArg)
{
...
_int CUI_EffectParttime::Tick(_float fTimeDelta)
{
...
void CUI_EffectParttime::Late_Tick(_float fTimeDelta)
{
	m_pGameInstance->Add_RenderGroup(CRenderer::RENDER_SUPERUI, this);
}

HRESULT CUI_EffectParttime::Render()
{
...
#ifdef _DEBUG
void CUI_EffectParttime::Render_IMGUI()
{
	//char ratio[16];
...
HRESULT CUI_EffectParttime::Add_Components()
{
	HRESULT hr(S_OK);
...
HRESULT CUI_EffectParttime::Bind_ShaderResources()
{
	HRESULT hr(S_OK);
...
CUI_EffectParttime* CUI_EffectParttime::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CUI_EffectParttime* pInstance = new CUI_EffectParttime(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CUI_EffectParttime"));
...
CGameObject* CUI_EffectParttime::Clone(void* pArg)
{
	CUI_EffectParttime* pInstance = new CUI_EffectParttime(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Clone : CUI_EffectParttime"));
		Safe_Release(pInstance);
	}

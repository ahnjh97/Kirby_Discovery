...
HRESULT CBulb::Initialize(void* pArg)
{
    ...
    m_eDeformType = DEFORM_BULB;
    m_eAnimIndex = BULB_FALL;
...
_int CBulb::Tick(_float fTimeDelta)
{
    if (true == m_bDead)
        return OBJ_DEAD;
    ...
    else if (m_eAnimIndex == BULB_FALL)
    {
        m_pControllerCom->FreeFall(m_pTransformCom, m_fTimeDelta, 3.f);
        m_fFallTime += m_fTimeDelta;

        if (m_pControllerCom->Is_Terrain() && m_fFallTime > 0.1f)
        {
            m_eAnimIndex = BULB_LANDING;
            m_fFallTime = 0.f;
        }
    }
    else if (m_eAnimIndex == BULB_LANDING)
    {
        m_pControllerCom->FreeFall(m_pTransformCom, m_fTimeDelta, 3.f);

        if (m_pModelCom->IsFinished())
            m_eAnimIndex = BULB_WAIT;
    }

    return OBJ_NOEVENT;
}

void CBulb::Late_Tick(_float fTimeDelta)
{
...
HRESULT CBulb::Render_LightDepth()
{
...
void CBulb::Collision(CCollisionCenter::CONTENT_TYPE eContent, CPhysXObject* pObject)
{
...
HRESULT CBulb::Add_Components()
{
    ...
    hr = __super::Add_Component(TEXT("Prototype_Component_Model_Bulb"),
...
CBulb* CBulb::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    ...
    {
        MSG_BOX(TEXT("Failed To Create : CBulb"));
...
CGameObject* CBulb::Clone(void* pArg)
{
    ...
    {
        MSG_BOX(TEXT("Failed To Clone : CBulb"));
        Safe_Release(pInstance);
    }

...
HRESULT CSurprisedBoard::Initialize(void* pArg)
{
	...
	m_pModelCom->Set_Animation(WAIT_L, 45.f, true, true);
...
void CSurprisedBoard::Collision(CCollisionCenter::CONTENT_TYPE eContent, CPhysXObject* pObject)
{
	if (eContent == CCollisionCenter::CONTENT_BODY)
	{
		...
		{
			Change_State(CSurprisedBoard::AWOOFY_DAMAGE, 50.f, false, true);
...
void CSurprisedBoard::SetUp_FSM()
{
	// FSM 상태 초기화
	m_pFSM = CFSM::Create();
